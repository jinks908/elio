...
impl App {
    ...
    pub(in crate::app) fn dispatch_action(&mut self, action: crate::config::Action) -> Result<()> {
        ...
        match action {
            ...
            Action::JumpUp => self.jump(-1),
            Action::JumpDown => self.jump(1),
    ...
    fn navigation_repeat_key(key: KeyEvent) -> Option<NavigationRepeatKey> {
        ...
        match crate::config::keys().action_for_key(key) {
            ...
            Some(crate::config::Action::JumpUp) => Some(NavigationRepeatKey::JumpUp),
            Some(crate::config::Action::JumpDown) => Some(NavigationRepeatKey::JumpDown),
